...
export default function Form({ onSubmit, loading }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (formData.generationMode === 'draft' && !formData.rawText.trim()) {
      setError('Raw text is required')
      return
    }
    if (formData.generationMode !== 'draft' && !formData.referenceFile && !formData.referenceText.trim()) {
      setError('Upload a template/reference document for rewrite/template mode')
      return
    }
      ...
      <form onSubmit={handleSubmit} className="flex-1 space-y-5 overflow-auto p-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              { id: 'template', label: 'Template Clone' },
            ].map((mode) => (
              <button
                key={mode.id}
                type="button"
                disabled={loading}
                onClick={() => setFormData((prev) => ({ ...prev, generationMode: mode.id }))}
                className={`rounded-lg border px-3 py-2 text-xs font-semibold ${formData.generationMode === mode.id ? 'border-teal-500 bg-teal-50 text-teal-800' : 'border-slate-200 text-slate-700'}`}
              >
                {mode.label}
              </button>
            ))}
          ...
          <div className="flex flex-wrap gap-2">
            {[1000, 3000, 5000, 10000].map((preset) => (
              <button
                ...
                type="button"
                disabled={loading}
                onClick={() => setFormData((prev) => ({ ...prev, wordCount: preset, customWordCount: '' }))}
                className={`rounded-full border px-3 py-1 text-xs font-semibold ${Number(formData.wordCount) === preset && !formData.customWordCount ? 'border-teal-500 bg-teal-50 text-teal-700' : 'border-slate-200 text-slate-600'}`}
              >
                {preset}
              </button>
            ))}
            <input
              ...
              onChange={(e) => setFormData((prev) => ({ ...prev, customWordCount: e.target.value, wordCount: e.target.value || prev.wordCount }))}
              placeholder="Custom"
              className="w-28 rounded-full border border-slate-200 px-3 py-1 text-xs font-semibold text-slate-700"
            />
          </div>
        </div>
        ...
        <div>
          <label className="field-label">
            Raw Text {formData.generationMode === 'draft' && <span className="text-rose-500">*</span>}
          </label>
          <textarea
            name="rawText"
            ...
            onChange={handleChange}
            disabled={loading}
            placeholder={formData.generationMode === 'draft' ? 'Paste your raw text here...' : 'Prompt rewrite instructions or topic change request...'}
            rows={5}
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 mb-2">Supports .txt, .doc, .docx, .pdf files as structural blueprint</p>
