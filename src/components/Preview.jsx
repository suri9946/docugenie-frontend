...
export default function Preview({ preview, locked, loading, documentId, progressMsg, targetWords, currentWords, progressPct }) {
  ...
  const wordCount = currentWords || preview.split(/\s+/).filter(Boolean).length
  ...
  const handleInitiatePayment = async (provider) => {
    try {
      ...
      setPaymentMeta(response.data)
      setPaymentStatus('processing')

      // Mobile flow: open UPI deep link directly
    ...
    <section className="app-panel flex h-full flex-col overflow-hidden">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-6 py-5">
        ...
        {locked ? (
          ...
          >
            {loading ? 'Downloading...' : 'Export DOCX'}
          </button>
        )}
      </div>

      {preview && !locked && (
        <div className="border-b border-slate-100 bg-white px-6 py-2 text-xs font-semibold text-slate-600 flex items-center gap-2">
          <span>Export:</span>
          <button onClick={() => downloadDocument(documentId, 'docx')} className="underline">DOCX</button>
          <button onClick={() => downloadDocument(documentId, 'pdf')} className="underline">PDF</button>
          <button onClick={() => downloadDocument(documentId, 'txt')} className="underline">TXT</button>
        </div>
      )}

      {/* Payment UI */}
      {locked && paymentStatus !== 'success' && showPaymentModal && (
        <div className="absolute inset-0 z-20 flex items-center justify-center bg-slate-900/40 p-4">
          <div className="w-full max-w-xl rounded-xl border border-slate-200 bg-white px-5 py-4 shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <p className="text-sm font-bold text-slate-900">Premium unlock</p>
                <p className="text-xs text-slate-500">UPI-only checkout (Google Pay, PhonePe, Paytm, Generic UPI).</p>
              </div>
              <button onClick={() => setShowPaymentModal(false)} className="text-sm text-slate-500">Close</button>
            </div>
            <div className="mb-4 grid grid-cols-1 gap-2 sm:grid-cols-2 text-xs">
              <div className="rounded-lg border border-slate-200 p-3">
                <p className="font-bold text-slate-800">Free</p>
                <p className="text-slate-500">Short previews and basic exports.</p>
              </div>
              <div className="rounded-lg border border-teal-200 bg-teal-50 p-3">
                <p className="font-bold text-teal-800">Premium</p>
                <p className="text-teal-700">Long-form generation, full exports, advanced rewrite/template tools.</p>
              </div>
            </div>
          {paymentStatus === 'idle' && (
            <div>
              ...
              <p className="text-xs text-slate-600 mb-4">
                {isMobile ? 'Choose your UPI app to open payment intent.' : 'Choose a UPI app. On desktop, scan QR with your phone to pay.'}
              </p>
            </div>
          )}
          ...
          {paymentStatus === 'processing' && (
            <div>
              ...
              {paymentMeta?.testMode && (
                <p className="mt-2 text-xs font-semibold text-amber-700">UPI_TEST_MODE enabled: manual verification is allowed for demo/testing.</p>
              )}
              {!isMobile && paymentMeta?.qrCodeUrl && (
                <div className="mt-4 rounded-lg border border-slate-200 p-3 text-center">
                  <p className="mb-2 text-xs font-semibold text-slate-700">Scan QR from your UPI app</p>
                  <img
                    src={paymentMeta.qrCodeUrl}
                    alt="UPI QR Code"
                    className="mx-auto h-44 w-44 rounded border border-slate-200"
                  />
                  {paymentMeta?.genericUpiLink && (
                    <button
                      type="button"
                      onClick={() => navigator.clipboard?.writeText(paymentMeta.genericUpiLink)}
          ...
          <div className="flex flex-1 items-center justify-center">
            <div className="text-center">
              ...
              <p className="mt-4 text-sm font-semibold text-slate-700">
                {progressMsg || "Preparing your preview..."}
              </p>
              <div className="mt-3 h-2 w-64 overflow-hidden rounded-full bg-slate-200">
                <div className="h-full bg-teal-600 transition-all duration-300" style={{ width: `${Math.max(8, progressPct || 0)}%` }} />
              </div>
              {(targetWords || currentWords) && (
                <p className="mt-2 text-xs text-slate-500">
                  Words: {currentWords || 0}{targetWords ? ` / ${targetWords}` : ''}
                </p>
              )}
            </div>
          </div>
